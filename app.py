...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
       num_messages,words,num_media_messages,num_links=helper.fetch_stats(select_user,df) 
    ...
    #    st.pyplot(fig)
       ...
       user_heatmap=helper.activity_heat_map(select_user,df)
       fig,ax=plt.subplots()
       ax=sms.heatmap(user_heatmap)
       st.pyplot(fig)
       ...
       st.title("Word Cloud")
       df_wc=helper.create_word_cloud(select_user,df)
       fig,ax=plt.subplots()
       ax.imshow(df_wc)
       st.pyplot(fig)

       most_common_df=helper.most_common_words(select_user,df)
       fig,ax=plt.subplots()
